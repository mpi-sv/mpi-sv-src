...
cl::opt<bool> ForkOnSchedule("fork-on-schedule",
		cl::desc("fork when various schedules are possible (defaul=disabled)"), cl::init(false));

cl::opt<unsigned int> MaxPreemptions("scheduler-preemption-bound",
		cl::desc("scheduler preemption bound (default=0)"), cl::init(0));

}

namespace klee {

bool Executor::checkGlobalAllDelayed() {
...
bool Executor::schedule(ExecutionState &state, bool yield) {
	...
	/// added by zhenbang to avoid redundant exploration
	if (checkGlobalAllDelayed()) {
		return false;
	}

	//added by Herman
//#define DEBUG_THR
//	LOG(INFO)<< "all threads pid/tid/enabled:"<<"***********************"<<"\n";
	int enabledCount = 0;
	for (ExecutionState::threads_ty::iterator it = state.threads.begin(); it != state.threads.end();
			it++) {
		if (it->second.enabled) {
			enabledCount++;
		}
#ifdef  DEBUG_THR
		LOG(INFO)<< it->second.getPid()<<"/"<<it->second.getTid()<<"/"<<it->second.enabled<<"\n";
#endif
...
//#undef DEBUG_THR
	...
	/*if (WithLibMPI && enabledCount == 0 && state.globalCommLog.allNodesBlocked()) {
		if (state.globalCommLog.check(state, 0)) {
	...
	if (enabledCount == 0) {
		if(WithLibMPI){
			...
			//kill other states that forked by wildcard receive
			//if(WithWildChoice)
	...
	//LOG(INFO)<< "current thread pid/tid/enabled:"<<oldIt->second.getPid()<<"/"<<oldIt->second.getTid()<<"/"<<oldIt->second.enabled<<"\n";
...
void Executor::executeThreadCreate(ExecutionState &state, thread_id_t tid, ref<Expr> start_function,
		ref<Expr> arg) {
...
void Executor::executeProcessExit(ExecutionState &state) {
	...
	VLOG(1) << "Terminating " << state.crtProcess().threads.size()
