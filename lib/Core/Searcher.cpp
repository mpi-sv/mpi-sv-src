...
void DFSSearcher::update(ExecutionState *current, const std::set<ExecutionState*> &addedStates,
		const std::set<ExecutionState*> &removedStates) {
	states.insert(states.end(), addedStates.begin(), addedStates.end());
	for (std::set<ExecutionState*>::const_iterator it = removedStates.begin(), ie =
		...
		if (es == states.back()) {
			...
			for (std::vector<ExecutionState*>::iterator it = states.begin(), ie = states.end();
					it != ie; ++it) {
				if (es == *it) {
					states.erase(it);
					ok = true;
...
ExecutionState &DirctedDFSSearcher::selectState() {
	ExecutionState * temp=states.back();
	/*if all the remained are delayed, then there is no need to explore, we return one with the label delayed=true, the the while loop in run will break*/
	//check whether the remained are all delayed, otherwise return the first undelayed encountering backward.
	bool allDelayed=true;
	vector<ExecutionState*>::iterator it;
	...
	if(allDelayed){
       return *states.back();
        ...
        return *result; //return the first undelayed state in backward direction.
	}
//	return *states.back();
}

void DirctedDFSSearcher::update(ExecutionState *current, const std::set<ExecutionState*> &addedStates,
		const std::set<ExecutionState*> &removedStates) {
//	LOG(INFO)<<"size of added states:" <<addedStates.size();
//	LOG(INFO)<<"size of removed states:" <<removedStates.size();
	states.insert(states.end(), addedStates.begin(), addedStates.end());
	for (std::set<ExecutionState*>::const_iterator it = removedStates.begin(), ie =
			removedStates.end(); it != ie; ++it) {
		ExecutionState *es = *it;
	...
	//fxj
	if (current != NULL) {
		ExecutionState *next = current->executor->mpiseDeadlockstate;
//		if(next!=NULL)
//		LOG(INFO)<<*next<<"\n";
		/*find the deadlockstate, remove it and insert it to the back of states!*/
		std::vector<ExecutionState*>::iterator nextit = find(states.begin(), states.end(),
				next);
		if (next != NULL && nextit != states.end()) {
			for (std::vector<ExecutionState*>::const_iterator it = states.begin(), ie =
					states.end(); it != ie; ++it) {
				ExecutionState *es = *it;
				if (es == current && next != states.back()) {
					//std::swap(next, states.back());
					states.erase(nextit);
...
void RandomSearcher::update(ExecutionState *current, const std::set<ExecutionState*> &addedStates,
		const std::set<ExecutionState*> &removedStates) {
	states.insert(states.end(), addedStates.begin(), addedStates.end());
	for (std::set<ExecutionState*>::const_iterator it = removedStates.begin(), ie =
			removedStates.end(); it != ie; ++it) {
...
WeightedRandomSearcher::WeightedRandomSearcher(Executor &_executor, WeightType _type) :
		executor(_executor), states(new DiscretePDF<ExecutionState*>()), type(_type) {
	switch (type) {
	case Depth:
		updateWeights = false;
		break;
...
void WeightedRandomSearcher::update(ExecutionState *current,
	...
	for (std::set<ExecutionState*>::const_iterator it = addedStates.begin(), ie = addedStates.end();
			it != ie; ++it) {
		ExecutionState *es = *it;
...
ExecutionState &BumpMergingSearcher::selectState() {
	...
	if (baseSearcher->empty()) {
		std::map<llvm::Instruction*, ExecutionState*>::iterator it = statesAtMerge.begin();
...
ExecutionState &MergingSearcher::selectState() {
	...
	for (std::set<ExecutionState*>::const_iterator it = statesAtMerge.begin(), ie =
			statesAtMerge.end(); it != ie; ++it) {
	...
	for (std::map<Instruction*, std::vector<ExecutionState*> >::iterator it = merges.begin(), ie =
			merges.end(); it != ie; ++it) {
		if (DebugLogMerge) {
			std::cerr << "\tmerge: " << it->first << " [";
			for (std::vector<ExecutionState*>::iterator it2 = it->second.begin(), ie2 =
					it->second.end(); it2 != ie2; ++it2) {
		...
		while (!toMerge.empty()) {
			...
			for (std::set<ExecutionState*>::iterator it = toMerge.begin(), ie = toMerge.end();
					it != ie; ++it) {
			...
			if (DebugLogMerge && !toErase.empty()) {
				...
				for (std::set<ExecutionState*>::iterator it = toErase.begin(), ie = toErase.end();
						it != ie; ++it) {
					if (it != toErase.begin())
						std::cerr << ", ";
					std::cerr << *it;
...
void MergingSearcher::update(ExecutionState *current, const std::set<ExecutionState*> &addedStates,
		const std::set<ExecutionState*> &removedStates) {
...
BatchingSearcher::BatchingSearcher(Searcher *_baseSearcher, double _timeBudget,
		...
		baseSearcher(_baseSearcher), timeBudget(_timeBudget), instructionBudget(_instructionBudget), lastState(
				0) {
...
void BatchingSearcher::update(ExecutionState *current, const std::set<ExecutionState*> &addedStates,
		const std::set<ExecutionState*> &removedStates) {
...
InterleavedSearcher::~InterleavedSearcher() {
	for (std::vector<Searcher*>::const_iterator it = searchers.begin(), ie = searchers.end();
			it != ie; ++it)
		delete *it;
}
