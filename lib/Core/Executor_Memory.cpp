...
void Executor::executeAlloc(ExecutionState &state, ref<Expr> size, bool isLocal,
	...
	if (ConstantExpr *CE = dyn_cast < ConstantExpr > (size)) {
		MemoryObject *mo = memory->allocate(&state, CE->getZExtValue(), isLocal,
	...
	bool success = solver->getValue(data::ALLOC_RANGE_CHECK, state, size,
			example);
	...
	while (example->Ugt(ConstantExpr::alloc(128, W))->isTrue()) {
		...
		bool res;
		bool success = solver->mayBeTrue(data::ALLOC_RANGE_CHECK, state,
	...
	if (fixedSize.second) {
		...
		bool success = solver->getValue(data::ALLOC_RANGE_CHECK,
				*fixedSize.second, size, tmp);
		...
		if (res) {
			...
			if (hugeSize.second) {
				...
				terminateStateOnError(*hugeSize.second,
						"concretized symbolic size", "model.err", info.str());
			}
		}
	}
...
void Executor::executeMemoryOperation(ExecutionState &state, bool isWrite,
	...
	ObjectPair op;
	bool resolved = false;
	bool chk_enable = true;

	// fast path: single in-bounds resolution
	solver->setTimeout(stpTimeout);
	if (!state.addressSpace().resolveOne(state, solver, address, op,
			resolved)) {
	...
	if (resolved) {
		...
		solver->setTimeout(stpTimeout);
		bool success = solver->mustBeTrue(data::SINGLE_ADDRESS_RESOLUTION,
				state, mo->getBoundsCheckOffset(offset, bytes), resolved);
		solver->setTimeout(0);
		if (!success) {
	...
	if (resolved) {
		...
		if (isWrite) {
			if (os->readOnly) {
				...
				//TODO:Here we add MPI write intercept, Herman

#ifdef SYNC_ERR_CHK

				chk_enable = state.globalCommLog.sync_chk_enable[state.crtThread().getTid()];

				string commtype;
				ConstantExpr *CE = dyn_cast < ConstantExpr > (address);
				if (chk_enable && CE) {
					state.globalCommLog.chkmap[state.crtThread().getTuid()]++;
					...
					uint64_t add = CE->getZExtValue(64);

					while (it != end) {

						if ((it->second.state == INIT	|| it->second.state == READY)
								&& (uint64_t) it->first <= add
								&& add	<= (uint64_t) it->first	+ it->second.size) {
							const InstructionInfo &ii = *state.prevPC()->info;
							commtype =	it->second.commtype == COMM_ISEND ?"send" : "recv";
							std::cout << "WRITE  buffer after" << commtype
									<< " & before wait! buffer:\t" << it->first
									<< "\t state \t" << it->second.state
									<< "\t size \t" << it->second.size
									<< " \t mo address \t" << (void *) (add)	<< "\n";
							if (ii.file != "")
								std::cout << "file: " << ii.file << "line "<< ii.line;

							terminateStateOnError(state, "sync error: ",	"isend.syncerr.err");
						}
						it++;
					}
				}
			...
			//Here we add MPI read intercept, Herman

#ifdef SYNC_ERR_CHK
//#ifdef ACESS_PLACEOPOT
			//This is too expensive.
			//std::vector<StackFrame>::iterator it2 =	state.stack().begin();
			//std::vector<StackFrame>::iterator itend2 =state.stack().end();
			//while (it2 != itend2) {
			//	if (it2->kf->function->getName().str().find("MPI", 0)!= -1)
			//		goto donotcheck;
			...
			if (chk_enable && CE) {
				...
				while (it != end) {
					...
					if ((it->second.state == INIT || it->second.state == READY)
							&& (uint64_t) it->first <= add
							&& add <= (uint64_t) it->first + it->second.size
							&& it->second.commtype == COMM_IRECV) {
						const klee::InstructionInfo *ii =it->second.ownerinfo;

						std::cout
								<< "Read  buffer after recv & before wait! buffer:\t"
								<< it->first << "\t state \t"
								<< it->second.state << "\t size \t"
								<< it->second.size;
						if(ii->file != "")
							std::cout << " file: " <<ii->file << " line "<< ii->line;

						std::cout<< " \t mo address \t"<< (void *) (add) << "\n";

						terminateStateOnError(state, "irecv.sync error:","syncerr.err");
					}
					it++;
				}
			}
   ...
   // ref<Expr> unboundCondition;
		...
		if (bound) {
			if (isWrite) {
				if (os->readOnly) {
					...
					ObjectState *wos = bound->addressSpace().getWriteable(mo,
							os);
	...
	if (unbound) {
		if (incomplete) {
			...
			//unboundCondition.get()->print(LOG(INFO));
			terminateStateOnError(*unbound,
...
void Executor::executeMakeSymbolic(ExecutionState &state,
	...
	const Array *array = new Array("arr" + llvm::utostr(++id), mo->size);
