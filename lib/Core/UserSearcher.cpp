...
cl::opt<bool> UseDirectedSearch("use-directeddfs-search");
...
cl::opt<bool> UseMerge("use-merge", cl::desc("Enable support for klee_merge() (experimental)"));

cl::opt<bool> UseBumpMerge("use-bump-merge",
		cl::desc("Enable support for klee_merge() (extra experimental)"));

cl::opt<bool> UseIterativeDeepeningTimeSearch("use-iterative-deepening-time-search",
...
cl::opt<double> BatchTime("batch-time",
		cl::desc("Amount of time to batch when using --use-batching-search"), cl::init(5.0));
}

bool klee::userSearcherRequiresMD2U() {
	return (WeightType == WeightedRandomSearcher::MinDistToUncovered
			|| WeightType == WeightedRandomSearcher::CoveringNew || UseInterleavedMD2UNURS
...
Searcher *klee::constructUserSearcher(Executor &executor, Searcher *original) {
	...
	if (!searcher) {
		...
		} else if (UseDirectedSearch) {
			searcher = new DirctedDFSSearcher();
		} else {
			searcher = new DFSSearcher();
		}
	}

	if (UseInterleavedNURS || UseInterleavedMD2UNURS || UseInterleavedRS || UseInterleavedCovNewNURS
		...
		if (UseInterleavedNURS)
			s.push_back(new WeightedRandomSearcher(executor, WeightedRandomSearcher::Depth));
		...
		if (UseInterleavedCovNewNURS)
			s.push_back(new WeightedRandomSearcher(executor, WeightedRandomSearcher::CoveringNew));

		if (UseInterleavedInstCountNURS)
			s.push_back(new WeightedRandomSearcher(executor, WeightedRandomSearcher::InstCount));

		if (UseInterleavedCPInstCountNURS)
			s.push_back(new WeightedRandomSearcher(executor, WeightedRandomSearcher::CPInstCount));

		if (UseInterleavedQueryCostNURS)
			s.push_back(new WeightedRandomSearcher(executor, WeightedRandomSearcher::QueryCost));
	...
	//LOG(INFO)<< "BEGIN searcher description\n";
	//searcher->printName(LOG(INFO));
	//LOG(INFO)<< "END searcher description\n";
