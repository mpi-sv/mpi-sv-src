...
int __Inj_MPI_Irecv(void *buf, int count, MPI_Datatype datatype, int source, int tag,	MPI_Comm comm, MPI_Request *request){
  ...
  if (source != MPI_ANY_SOURCE) {
    ...
    if (i > 10) {
     return MPI_Irecv(buf, count, datatype, 1, tag, comm, request);
    } else {
     return MPI_Irecv(buf, count, datatype, MPI_ANY_SOURCE, tag, comm, request); // we just mutant it to receive from 1
    }
  }
}
