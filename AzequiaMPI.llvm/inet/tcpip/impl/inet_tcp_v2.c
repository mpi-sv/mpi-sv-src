		...
		for (i = 0; i <= maxi; i++) {
			...
			if (FD_ISSET(v_client[i], &rset)) {
				...
				if (size == 0) {
					...
					//	if (0 > (size = readpacket(v_client[i], azqHdr->RrvDstAddr,
					//						hdr->FrgmtSize - HEADER_NET_SZ)))                   { printf("readpacket(1)\n"); fflush(stdout); goto exception; }
					//	azqHdr->Mode |= MODE_NO_COPY;
					//} else {
						if (0 > (size = readpacket(v_client[i], (char *)&packet[size],
											hdr->FrgmtSize - HEADER_NET_SZ)))                   { printf("readpacket(2)\n"); fflush(stdout); goto exception; }
					//}
