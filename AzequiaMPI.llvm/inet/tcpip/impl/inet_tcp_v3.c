...
/* Size of input and output buffers of tcp sockets */
#define TCP_RCVBUF				(129 * 1024)
#define TCP_SNDBUF				(129 * 1024)
...
/* INET maximum frame size for fragmenting messages. Calculated to be a
   multiple of 8. Usually equal to the tcp socket output buffer size */
#define INET_FRAME_SIZE   TCP_SNDBUF
...
static int CONTADOR_WRITEV = 0;
static int CONTADOR_READV = 0;
...
static void *accept_body (void *params) {
  ...
  int                 listenfd = *((int *) params);
  int                 on = 1;
	RxMchnInfo         *minfo;

	fd_set rset, allset;
	int maxfd,
			maxi,			/* max index in client array */
			nready;		/* number of events pending of processing */

  char               *packet;
  INET_header        *hdr;
  Header             *azqHdr;
  int                 i, size, v_client[FD_SETSIZE];
	int									tcp_rcvbuf = TCP_RCVBUF;
  ...
  while (1) {
		...
		//printf("Me voy a bloquear en select!!\n"); fflush(stdout);

		// verificar que nready no vale 0...
		if (0 >= (nready = select(maxfd + 1, &rset, NULL, NULL, NULL)))      { printf("select\n"); fflush(stdout); goto exception; }


		//printf("Select: new event occurrs!!\n"); fflush(stdout);

		if (FD_ISSET(listenfd, &rset)) {
			minfo = &RxMchnList[RxMchnAlloc];	/* we already have v_client with client socket stored. do we need it? */
			addrlen = sizeof(peerAddr);

			//printf("New connection arrived! || socket %d || sa = %d, %s, %d\n", minfo->RecvSock, peerAddr.sin_family, inet_ntoa(peerAddr.sin_addr), ntohs(peerAddr.sin_port)); fflush(stdout);

			/* If client aborts connection after select() and before accept(), we will block ourself in accept(). To solve: chapter 16.6 */
			if (0 > (minfo->RecvSock = accept(listenfd, (struct sockaddr *)&peerAddr, &addrlen))) { printf("accept\n"); fflush(stdout); goto exception; }
			if (0 > setsockopt(minfo->RecvSock, SOL_SOCKET, SO_REUSEADDR, &on, sizeof(on)))     	{ printf("setsockoption\n"); fflush(stdout); goto exception; }
//      if (0 > setsockopt(minfo->RecvSock, SOL_SOCKET, SO_RCVBUF, (char *)&tcp_rcvbuf, sizeof(int))) { goto exception; }
			/*if (0 > launch_task(&minfo->ScanTask, INET_PRIORITY, scan, minfo))          goto exception;*/
			RxMchnAlloc++;

			//printf("New connection arrived!! [socket %d is from machine %d]\n", minfo->RecvSock, peerAddr.sin_addr.s_addr); fflush(stdout);


			for (i = 0; i < FD_SETSIZE; i++)
				if (v_client[i] < 0) {
					v_client[i] = minfo->RecvSock;
					break;
				}
			if (i == FD_SETSIZE)																							{ printf("i==FD_SETSIZE\n"); fflush(stdout); goto exception; } /* What do we do here? */

			FD_SET(minfo->RecvSock, &allset);
			if (minfo->RecvSock > maxfd)
				maxfd = minfo->RecvSock;
		...
		for (i = 0; i <= maxi; i++) {
			...
			if (FD_ISSET(v_client[i], &rset)) {

				//printf("New data arrived!!\n"); fflush(stdout);

				/* We assume client will send us all data. If not, there will be
				 * an 'denial of service' scenery. Also, if all goes well, we
				 * probably need to block in read() many times. This time won't
				 * be productive. */

				ELB_newElem(elb, (void *)&packet);

				if (0 > (size = readpacket(v_client[i], (char *)&packet[0],
									sizeof(INET_header) + HEADER_NET_SZ)))                { excpn = INET_E_INTEGRITY;
                                                                          goto exception; }
				if (size == 0) {
					/* Connection closed by remote peer */
					close(v_client[i]);
					FD_CLR(v_client[i], &allset);
					v_client[i] = -1;
				} else {
					...
					/* Se evita el caso de mensajes grandes que no llevan RRV, ya que el receptor puede no estar preparado */


					if ((!(azqHdr->Mode & MODE_RRV_ACK)) && (azqHdr->RrvDstAddr != NULL)) {
						if (0 > (size = readpacket(v_client[i], azqHdr->RrvDstAddr, hdr->FrgmtSize - HEADER_NET_SZ)))                   { printf("readpacket(1)\n"); fflush(stdout); goto exception; }
						azqHdr->Mode |= MODE_NO_COPY;
					} else {
						if (0 > (size = readpacket(v_client[i], (char *)&packet[size], hdr->FrgmtSize - HEADER_NET_SZ)))                { printf("readpacket(2)\n"); fflush(stdout); goto exception; }
					}

					/* Built a INET iov struct */
					payload.Data  = packet + sizeof(INET_header);
					payload.Size  = ((INET_header *)packet)->FrgmtSize;

					/* Deliver the message to the correct protocol upper level */
					protocol      =  ((INET_header *)packet)->Protocol;
					is_last_frgmt =  ((INET_header *)packet)->Mode & INET_LAST_FRAGMENT;

					DBG_PRNT((stdout, "[%d] INET: Delivering MSG from machine  %x  to protocol  %d  \n", cpuId, ((INET_header *)payload.Data)->SrcMchn, protocol));

					/* Deliver the message to upper level */
					...
					/* If upper level copied the message, the packet is freed, else only a reference is in the
...
/*#ifdef __INET_DEBUG
  ...
  //printf("accept_body(). End.\n"); fflush(stdout);
  accept_ret = 0;
  return (&accept_ret);

exception:

  printf("accept_body(). Exception.\n"); fflush(stdout);
...
/*  if (0 > setsockopt(minfo->SendSocket, SOL_SOCKET, SO_SNDBUF, (char *)&tcp_sndbuf, sizeof(int)))
                                                                               {excpn = INET_E_INTEGRITY;
                                                                                goto exception;}*/
...
//cuando hago un pthread_cancel sobre el hilo accept y este se encuentra bloqueado en select, entonces el hilo muere y pthread_cancel retorna con 0.
//cuando hago un shutdown o un close sobre un socket, se cierra la conexión y en el lado remoto pueden suceder dos situaciones:
/*1. Que la maquina remota todavia no haya llamado a pthread_cancel sobre la tarea accept. En este caso, al detectar el cierre de conexion, la tarea accept se sale de select
   creyendo que hay una lectura. Hace el primer read y este devuelve 0 e itera indefinidamente. Cuando se invoca cancel en este estado, parece que no cancela a la tarea accept.
2. Que la maquina remota ya haya llegado a cancel. En este caso no hay problema, pues se mata al hilo accept, que esta bloqueado en select, y se cierran las conexiones y todo termina bien.
*/

//	if (cpuId == 0)
//  	sleep(5);

//  fprintf(stdout, "INET_finalize: Vamos a cancelar accept!!!\n"); fflush(stdout);

//	if (cpuId == 0)
//  	sleep(1);

printf("----------->%d\n", CONTADOR_WRITEV); fflush(stdout);
printf("---##-------->%d\n", CONTADOR_READV); fflush(stdout);
  ...
  shutdown(general_sockfd, SHUT_RDWR);
  ...
  pthread_mutex_destroy(&MchnTableLock);
    ...
    //pthread_join(rxMchnInfo->ScanTask, NULL);
		...
		/* Probably we're goint to close already closed sockets. Ignore return value */
...
//		printf("iov[2].iov_len=%d bytes\n", iov[2].iov_len); fflush(stdout);
