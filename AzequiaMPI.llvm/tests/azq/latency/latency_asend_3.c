  ...
  for (k = 0; k < 100000000; k++) {
    ...
    if (myid == (numprocs - 1)) {
      ...
      for (i = 0; i < numprocs - 1; i++) {
        ...
        //if (0 > nblk_send(&dst, (char *)&buf, sizeof(int), 99, &rqst[i])) {err = -3; goto error;}
        if (0 > asend(&dst, (char *)&buf, sizeof(int), 99, &req[i])) {err = -3; goto error;}
      }
      ...
      for (i = 0; i < (numprocs - 1); i++) {
        if (0 > waitany(req, numprocs - 1, &idx, &status)) {err = -1; goto error;}
