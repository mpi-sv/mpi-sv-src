...
/*
    if(0 > (excpn = send(&dst, (char *)&value[0], bufSize, 99))) {
      fprintf(stderr, "\n:: S :::::::::: Sender(%x). [%d  %d]: Exception %d :::::::::: n", (int)(THR_self()), gix, myid, excpn);
      free(value);
      return(excpn);
    }
...
int do_receiver (int bufSize) {
  ...
  req_ptr = &req;
  if(0 > (excpn = arecv(&src, (char *)&buf0[0], bufSize, 99, req_ptr))) {
    fprintf(stderr, "\n:: S :::::::::: Sender(%x). [%d  %d]: Exception %d :::::::::: n", (int)(THR_self()), gix, myid, excpn);
    free(buf0);
    return(excpn);
  }

  if(0 > (excpn = timed_recv(&src, (char *)&buf1[0], bufSize, 99, &status, COM_FOREVER))) {
