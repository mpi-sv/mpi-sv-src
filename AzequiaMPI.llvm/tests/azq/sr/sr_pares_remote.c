...
int do_sender (int bufSize) {
  ...
  while (1) {
    ...
    fprintf(stdout, "Sender(%x). sended buffer of size %d.  \n", (unsigned int)(THR_self()), bufSize);
    fprintf(stdout, "%lf ====\n", abstime()); fflush(stdout);
...
int do_receiver (int bufSize) {
  ...
  fprintf(stdout, "Receiver(%x). received buffer of size %d from %d.  \n", (unsigned int)(THR_self()), status.Count, status.Src.Rank);
  fprintf(stdout, "%lf ====\n", abstime()); fflush(stdout);
