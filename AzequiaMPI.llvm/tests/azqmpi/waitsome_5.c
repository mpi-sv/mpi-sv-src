...
#define MSG_PER_STREAM   16
//#error  (MSG_PER_STREAM > MAX_REQUESTS)
...
int main (int argc, char **argv) 
{
  ...
  if(MSG_PER_STREAM > MAX_REQUESTS) {
    printf("Error: MSG_PER_STREAM > MAX_REQUESTS\n"); 
    exit(1);
  }
  MPI_Init(NULL, NULL);
  ...
  if (myid == 0) {
    ...
    for (i = 0; i < COUNT; i++)
      recvbuf[i] = myid + i;
    for (i = 0; i < MSG_PER_STREAM; i++)
      MPI_Recv_init (recvbuf, COUNT, MPI_INT, 0, myid * 10, MPI_COMM_WORLD, &request[i]);
    for (i = 0; i < MSG_PER_STREAM; i++) {
    ...
    total_cnt = 0;
    outcount = 0;
    while ((total_cnt < MSG_PER_STREAM) && (outcount != MPI_UNDEFINED)) {
      MPI_Waitsome(MSG_PER_STREAM, request, &outcount, idx, status);
      if(outcount != MPI_UNDEFINED) {
          total_cnt += outcount;
      }
    }
