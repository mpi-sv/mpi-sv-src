...
int main (int argc, char **argv) {
  ...
  /* Generate membershipKey in the range [0, 1] */
  //membershipKey = myid % 2;
  if (myid >= 8)  membershipKey = 1;
  else            membershipKey = 0;
  ...
  if (membershipKey == 0) {
    ...
    MPI_Intercomm_merge(myFirstComm, membershipKey, &newintracomm);
  ...
  if (myid == 0) {
    ...
    MPI_Comm_test_inter(newintracomm, &flag);
    fprintf(stdout, "newintracomm is an %s\n", flag ? "Intercommunicator" : "Intracommunicator");

    MPI_Comm_compare(MPI_COMM_WORLD, newintracomm, &flag);
    fprintf(stdout,"MPI_COMM_WORLD vs newintracomm (MPI_CONGRUENT):"); compare(flag);
  }
