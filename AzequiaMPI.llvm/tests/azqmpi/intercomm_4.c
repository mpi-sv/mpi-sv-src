...
int main (int argc, char **argv) {
  ...
  MPI_Comm_dup(myFirstComm, &newintercomm);
  ...
  if (myid == 0) {
    ...
    MPI_Comm_test_inter(newintercomm, &flag);
    fprintf(stdout, "newintercomm is an %s\n", flag ? "Intercommunicator" : "Intracommunicator");

    MPI_Comm_compare(newintercomm, myFirstComm, &flag); /* MPI_CONGRUENT */
	  fprintf(stdout,"newintercomm vs myFirstComm (MPI_CONGRUENT):"); compare(flag);
  ...
  if (myid == 0) {
    buf[0] = 98;
    MPI_Send(buf, 1, MPI_INT, 0, 45, newintercomm);
  }
  if (myid == 1) {
    MPI_Recv(buf, 1, MPI_INT, 0, 45, newintercomm, &status);
