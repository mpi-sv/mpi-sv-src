...
static inline int locateService(int svc, int *mchn, int *gix) {
  ...
  DBG_PRNT((stdout, "locateService: [Port %d, Mchn %d]\n", svc, *mchn));
...
int rpc_send(const Addr_t dst, char *buff, int cnt, int mode) {
  ...
  if(mode & RPC_SVR) {
    ...
    DBG_PRNT((stdout, "rpc_send(%p): To machine %d \n", me, dst->Rank));
    dstMchn = dst->Rank;
    dst->Rank = 0;
  }

  DBG_PRNT((stdout, "rpc_send(%p): To [G: %x, R: %d, M: %d]:\n", me, dst->Group, dst->Rank, dstMchn));
  ...
  rqst = &(me->SyncRqst);
  RQST_init(rqst, RQST_SEND | RQST_SYNC | RRV_msg(RRV_MODE, cnt),
                  dst,
                  0,
                  buff,
                  cnt,
...
int rpc_recv(const Addr_t src, char *buff, int cnt, Status *status, unsigned timeout) {
  ...
  /* 3. Wait */
  if (0 > (excpn = timed_wait(&rqst, status, timeout)))                        goto exception;
...
int RPC_init(void) {
  ...
  if(0 > (excpn = INET_subscribe(deliver, RPC_PROTOCOL)))                      goto exception;

  DBG_PRNT((stdout, "RPC_init: Subscribed %p for protocol %d. End\n", deliver, RPC_PROTOCOL));
...
int RPC_trans(Hdr_t hdr, void *buff1, int cnt1, void *buff2, int cnt2) {
  ...
  DBG_PRNT((stdout, "\tRPC_trans: init\n"));
...
exception:
  XPN_print(excpn);
  DBG_PRNT((stdout, "RPC_getServer: End\n"));
  return(excpn);
}
