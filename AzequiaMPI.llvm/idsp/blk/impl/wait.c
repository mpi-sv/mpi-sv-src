...
#define self()        ((Thr_t)pthread_getspecific(key))
    ...
    |                                                                    |
     \____________/  ___________________________________________________/
                ...
                /_/
               */
int timed_wait(Rqst_t *rqst, Status *status, unsigned timeout) {

  int      excpn;
  Thr_t    me          = self();
  static
  char    *where       = "timed_wait";

  DBG_PRNT((stdout, "\ntimed_wait(%p). Rqst: %p with state %x\n", me, *rqst, (*rqst)->State));

  LOCK(me);
//(*rqst == AZQ_RQST_NULL) || RQST_isInactive(*rqst)
  if((*rqst == AZQ_RQST_NULL) )                       {excpn = COM_E_INTEGRITY;
                                                                       RQST_setEmptyStatus(status);
																			    UNLOCK(me);
                                                                                goto exception;}

  if(RQST_isInactive(*rqst))                      	  {excpn = AZQ_E_INTERN;
                                                                      RQST_setEmptyStatus(status);
  																			    UNLOCK(me);
                                                                                  goto exception;}
  ...
  switch((*rqst)->State) {
    ...
    case RQST_PENDING:
      ...
      if (0 > (excpn = TIMEDWAIT(me, me, timeout)))                            {UNLOCK(me);
                                                                                goto exception;}
...
exception:
 // XPN_print(excpn);
 printf("wait in wait.c, exception: %d\n",excpn);
