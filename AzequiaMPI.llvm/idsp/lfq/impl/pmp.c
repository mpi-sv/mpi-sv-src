...
int PMP_init() {
  ...
  if (0 > sem_close(pmp_sync))                                                 goto exception_2;
...
#endif  
exception_2:
  fprintf(stdout, "EXCP 2 \n");
  perror("ERROR: ");
  pthread_mutex_destroy(&table.Lock);
exception_1:
    fprintf(stdout, "EXCP 1 \n");
  AZQ_FREE(table.Cpl);
exception_0:
    fprintf(stdout, "EXCP 0 \n");
  XPN_print(excpn);
  return(excpn);
}
