  ...
  return done;
    ...
    |                                                                    |
     \____________/  ___________________________________________________/
                ...
                /_/
               */
int testsome(Rqst_t *rqst, int count, int *index, RQST_Status *status, int *outcount)
{
  Thr_t      me          = self();
  int        slideNr, 
             i, 
  ...
  if(slideNr > SLIDE_MAX)                                                      {excpn = AZQ_E_EXHAUST;
                                                                                goto exception;}
  /* 2. Activate the bits corresponding to NULL requests passed to testsome
  ...
  *outcount = 0;
  ...
  if(j) {
    *outcount = j;
    DBG_PRNT((stdout, "testsome(%p) with %d requests covered: End\n", self(), *outcount));
    goto retorno;
  }  
