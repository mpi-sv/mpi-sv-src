...
void rpq_print (Header_t hdr) {
  ...
  fprintf(stdout, "\t\tSource THR  0x%x\n", hdr->SrcThr);
...
{
//Rqst_t targetRqst = (Rqst_t)elem;
#ifdef __RPQ_DEBUG
//rqst_print((Rqst_t)elem);
#endif
//if(!elem) panic("rpq_match");
//fprintf(stdout, "rpq_match(%p): The request to be matched is\n", targetRqst->Owner);
//RQST_print(targetRqst);
