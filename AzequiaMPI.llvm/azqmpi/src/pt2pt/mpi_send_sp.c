...
int MPI_Ssend_init (void *buf, int count, MPI_Datatype datatype, int dest,
...
#ifdef DEBUG_MODE
  fprintf(stdout, "MPI_Ssend_init (start)\tProcess: 0x%x\n", PCS_self());
#endif
  ...
  if (dtypeIsContiguous(datatype)) {
    ...
    CALL_FXN (pssend_init(rank,                                     /* Dest. address */
                          buf,                                      /* message       */
                          size,                                     /* Size          */
...
#ifdef DEBUG_MODE
  fprintf(stdout, "MPI_Ssend_init (end)  \tProcess: 0x%x\n", PCS_self());
#endif
...
mpi_exception:
  return commHandleError (comm, mpi_errno, "MPI_Ssend_init");
}
