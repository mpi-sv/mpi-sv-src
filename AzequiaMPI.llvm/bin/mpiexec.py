...
mpiexec [global args] [local args] executable [args]
   where global args may be
      -l                           # line labels by MPI rank
      -gdb                         # run procs under gdb
      -v                           # display some environment information
   and local args may be
      -n <n> or -np <n>            # number of nodes to start
      -perhost <n>                 # number of nodes to start per host
      -bysocket                    # bind nodes by socket in R.R.
      -no-binding                  # No bind nodes to cores

Example:
   mpiexec -l -n 10 -perhost 4 -bysocket cpi 100
"""
...
def mpiexec():
    ...
    if parmdb['-gdba']:
	    ...
	    handle_local_argset(conSock,localArgSets[k],machineFileInfo,msgToMPD)
...
# RICO: Incluir verbose mode
        elif garg == '-v':
            parmdb[('cmdline','MPIEXEC_VERBOSE')] = 1
            argidx += 1
...
def handle_local_argset(conSock,argset,machineFileInfo,msgToMPD):
    global parmdb, nextRange, appnum, recvTimeout
    validLocalArgs  = { '-n' : 1, '-np' : 1, '-host' : 1, '-path' : 1, '-wdir' : 1,
                        '-soft' : 1, '-arch' : 1, '-umask' : 1,
			'-envall' : 0, '-env' : 2, '-envnone' : 0, '-envlist' : 1, '-perhost' : 1, 
			'-bysocket' : 0, '-no-binding' : 0 }
    host   = parmdb['-ghost']
    wdir   = parmdb['-gwdir']
    ...
    while argidx < len(argset):
        ...
        if argset[argidx] == '-n' or argset[argidx] == '-np':
            if len(argset) < (argidx+2):
                ...
                usage()
            nThreads = argset[argidx+1]
            if not nThreads.isdigit():
                print '** non-numeric arg to -n: %s' % nThreads
                usage()
            ...
            #nProcs = int(nProcs)
            argidx += 2
        elif argset[argidx] == '-perhost':
            if len(argset) < (argidx+2):
                print '** missing arg to -perhost'
                usage()
            perHost = argset[argidx+1]
            if not perHost.isdigit():
                print '** non-numeric arg to -perhost: %s' % perHost
                usage()
            perHost = int(perHost)
            usingPerHost = 1
            argidx += 2
        elif argset[argidx] == '-bysocket':
            bySocket = 1
            if (noBinding == 1):
                print '** Incompatible arguments -no-binding and -bysocket'
                usage()
                sys.exit(-1)
            argidx += 1
        elif argset[argidx] == '-no-binding':
            noBinding = 1
            if (bySocket == 1):
                print '** Incompatible arguments -no-binding and -bysocket'
                usage()
        ...
        #####################################################
        # Get the total number of ring's machines right now #
        #####################################################
        nMachines = get_machine_nr(conSock)

        #####################################################
        # Check for incoherent argument combinations        #
        #####################################################
        if usingPerHost == 1:
          if nThreads > (nMachines * perHost):
            print 'Too many nodes for %i machines and %i node(s) per host' % (nMachines, perHost)
            sys.exit(-1)

        ###########################################################
        # Set up how many processes will be started. This means   #
        # number of machines used too, because Azequia always     #
        # launchs one process per machine.                        #
        ###########################################################
        if nThreads < (nMachines * perHost):
          nProcs = (nThreads + (perHost-1)) / perHost
        else:
          nProcs = nMachines
    ...
    while loRange <= argsetHiRange:
        ...
        #############################################
        # Set up the Azequia's parameters right now #
        #############################################

        # 0. HWLOC LIBRARY PATH (set up by makefile.am)
        envToSend['LD_LIBRARY_PATH'] =

        # 1. AZQMPI_NODES
        envToSend['AZQMPI_NODES'] = str(nThreads)

        # 2. AZQMPI_PERHOST
        envToSend['AZQMPI_PERHOST'] = str(perHost)

        # 3. AZQMPI_HOSTS
        envToSend['AZQMPI_HOSTS'] = str(nProcs)

        # 4. AZQMPI_BYSOCKET
        envToSend['AZQMPI_BYSOCKET'] = str(bySocket)

        # 4. AZQMPI_NO_BINDING
        envToSend['AZQMPI_NO_BINDING'] = str(noBinding)

        if parmdb['MPIEXEC_VERBOSE']:
            print '\nApplication: <%s>. ' % msgToMPD[('execs')]
            print '  Date: %s.\n' % __date__
            print '  Running with parameters: '
            print '    %s\t Host(s).' % nProcs
            print '    %s\t Node(s).' % nThreads
            print '    %s\t Node(s) per host (Round Robin).' % perHost
            if bySocket == 1:
              print '    Binding by socket.'
            if noBinding == 1:
              print '    NOT binding nodes to cores'
...
########################################################################
# Get the total number of machines of the ring
########################################################################
def get_machine_nr(conSock):
    machine_nr = 0
