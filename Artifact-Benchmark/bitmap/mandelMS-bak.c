...
Pixel cal_pixel(Compl c, int size, int rank) {
    ...
    //result.r = z.real 
    //result.r = rank*rcolors & 0xFF0000mak
    return result;
}
...
void worker(int linebytes) {
    ...
    while (status.MPI_TAG != TERM_TAG) {
        ...
        printf("---%d-----MPI_Send----------%d------%p-----", rank, linebytes, colors);
...
int main(int argc, char **argv) {
    ...
    //block
    FILE *fp = 0;
    if ((fp = fopen("/home/zbchen/mpisv/src/MPISV/Artifact-Benchmark/bitmap/dot.bmp", "r+")) == NULL) {
    ...
    if (rank != 0)
        ...
        //block
        bmp.iheader.height = Y_RESN;//t


        bmp.iheader.planes = 1;
        bmp.iheader.isize = linebytes * bmp.iheader.height; //t
        bmp.header.offset = 54;
        bmp.header.size = bmp.header.offset + bmp.iheader.isize; //t
        ...
        for (k = 1; k < size; k++) {
            colors = (Pixel *) malloc(linebytes * sizeof(Pixel));
            MPI_Recv(&colors[0], linebytes, MPI_BYTE, k, MPI_ANY_TAG, MPI_COMM_WORLD, &status);
        }
        for (k = 1; k < size; k++) {
            MPI_Send(&k, 1, MPI_INT, k, TERM_TAG, MPI_COMM_WORLD);
        }
    }
